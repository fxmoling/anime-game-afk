...
```bash
timestamp=$(node "c:/Users/Administrator/Desktop/anime-game-afk/.claude/get-shit-done/bin/gsd-tools.cjs" current-timestamp full --raw)
```
...
<step name="commit">
```bash
node "c:/Users/Administrator/Desktop/anime-game-afk/.claude/get-shit-done/bin/gsd-tools.cjs" commit "wip: [context-name] paused at [X]/[Y]" --files [handoff-path] .planning/HANDOFF.json
```
</step>
